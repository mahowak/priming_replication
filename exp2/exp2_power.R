# This script runs the Exp 2 power analysis, as described
...
simulate.df = function(ns, ni, beta1) {
  nsubj = ns
  ...
  for (it in seq(1, 2)) {
    ...
    for (curnum in seq(startpoint, ns, 100)) {
      if (curnum == startpoint | (bf.new.null > (1/6) & bf.new.null < 6)) {
        ...
        print(a)
        write.table(a, file="exp_2_power_analysis.csv", append = T, sep=",", row.names=F, col.names=F)
      }
    }
  }
}
